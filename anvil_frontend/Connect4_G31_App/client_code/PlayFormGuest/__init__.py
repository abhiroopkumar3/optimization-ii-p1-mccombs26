from ._anvil_designer import PlayFormGuestTemplate
from anvil import *
...
class PlayFormGuest(PlayFormGuestTemplate):
  ...
  def _collect_grid_labels(self):
    ...
    for r in range(ROWS):
      ...
      if len(row_cells) != COLS:
        raise Exception(
          f"{row_flow_name} must contain exactly {COLS} Labels (found {len(row_cells)})"
        )
  ...
  def play_col(self, col):
    ...
    if not (0 <= int(bot_col) < COLS) or self.board[0][int(bot_col)] != 0:
      self.status_label.text = (
        "⚠️ Bot returned an invalid move. Ending game as No Result."
      )
    ...
    if win_cells:
      self.render_board(highlight_cells=win_cells)
      self.status_label.text = (
        f"🤖 Bot wins (played {bot_col}). Click 'New Game' to try again."
      )
      self.end_game("loss")
  ...
  def col0_button_click(self, **event_args):
    self.play_col(0)

  @handle("col1_button", "click")
  def col1_button_click(self, **event_args):
    self.play_col(1)

  @handle("col2_button", "click")
  def col2_button_click(self, **event_args):
    self.play_col(2)

  @handle("col3_button", "click")
  def col3_button_click(self, **event_args):
    self.play_col(3)

  @handle("col4_button", "click")
  def col4_button_click(self, **event_args):
    self.play_col(4)

  @handle("col5_button", "click")
  def col5_button_click(self, **event_args):
    self.play_col(5)

  @handle("col6_button", "click")
  def col6_button_click(self, **event_args):
    self.play_col(6)
  ...
  def end_game_button_click(self, **event_args):
    ...
    self.status_label.text = (
      "Game ended with no result. Click 'New Game' to start again."
    )
    self.end_game("no_result")
  ...
  def gameplay_instructions_button_click(self, **event_args):
    ...
    time.sleep(0.5)
    open_form("InstructionsFormGuest")
